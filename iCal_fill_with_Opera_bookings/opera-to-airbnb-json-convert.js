...
// This script reads Opera PMS data from a JSON file, extracts rooms with Airbnb listings and reservations,
// generates iCal files for each room, and hosts them via a web server for Airbnb calendar syncing.
// It can be run manually or scheduled to run at regular intervals to keep Airbnb calendars up to date.
// Note: This script requires the 'ical-generator' package. Install it using 'npm install ical-generator'.
...
const config = {
  // Path to your Opera JSON data file (use absolute path)
...
function createIcalFiles(rooms) {
  ...
  rooms.forEach(room => {
    ...
    const calendar = ical({
      ...
      prodId: `-//Opera-Airbnb-Sync//${room.property}_${room.roomNumber}//EN`,
    ...
    // Generate filename based on room number
    const filename = `${room.roomNumber}.ics`;
    ...
    console.log(`Generated iCal for room ${room.roomNumber} with ${room.reservations.length} reservations`);
  });
...
function hostIcalFiles(icals) {
  const server = http.createServer((req, res) => {
    ...
    if (pathname === '' || pathname === 'index.html') {
      ...
      const roomNumbers = Object.keys(icals).sort((a, b) => a.localeCompare(b, undefined, { numeric: true }));
      const links = roomNumbers.map(roomNum => {
        const ical = icals[roomNum];
