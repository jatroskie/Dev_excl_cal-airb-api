...
# RabbitMQ connection (moved outside function to persist)
try:
...
def fetch_and_queue_emails():
    """
    Perpetually monitor Gmail inbox for unread 'Pending: Reservation' emails and queue them in RabbitMQ.
    """
        ...
        while True:
            try:
                ...
                if not messages:
                    ...
                    logging.info(f"Found {len(messages)} new unread emails to queue.")
                ...
                time.sleep(60)  # Retry after a delay to avoid rapid failure loops
