...
def extract_body_fields(body):
    ...
    # Priority: Extract name from "Respond to [name]’s Request"
    name_pattern = r"Respond to (.+?)’s Request"
    for line in lines:
        match = re.search(name_pattern, line)
        if match:
            fields["client_name"] = match.group(1).strip()
            logging.info(f"Extracted client_name from body: {fields['client_name']}")
            break
...
def callback(ch, method, properties, body):
    ...
    if any(keyword in subject.lower() for keyword in ['pending: reservation', 'booking', 'reservation']):
        ...
        try:
            ...
            # Use body-extracted name if available, else fallback
            ...
            # Send reply using the extracted name
