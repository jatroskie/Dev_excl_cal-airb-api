# Setup Guide: iCal to Firestore with Manual Guest Information Collection

This guide walks you through setting up an automated system to sync your Airbnb and Booking.com calendars to Firestore with a separate process for collecting guest information manually or from emails.
...
3. **Install required dependencies:**
   ```
   npm install firebase-admin node-ical axios node-cron express ejs
   ```
...
5. **Create project files:**
   - Copy the code from the provided artifacts into separate files:
     - `ical-firestore-sync.js` - Core syncing functionality
     - `firestore-setup.js` - Database setup
     - `reservation-dashboard.js` - Web dashboard
     - `guest-info-collector.js` - CLI tool for guest info collection
...
3. **Run the guest information collector tool:**
   ```
   node guest-info-collector.js
   ```
   This interactive CLI tool allows you to:
   - List reservations needing guest information
   - Manually enter guest details
   - Parse guest information from saved email content

4. **For production deployment:**
   - Consider running the web dashboard as a service using PM2 or similar
   - Set up proper authentication for the dashboard
   - Schedule the iCal sync using cron or a cloud scheduler

d ensure they're publicly accessible
