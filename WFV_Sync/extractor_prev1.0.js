// extractor.js - Refined results wait locator to be more specific (target the results grid table), avoid strict mode issues with .first(), and fixed 'page' reference to 'this.page' in retry delay
...
class OperaExtractor {
  ...
  async downloadReservationCSV(roomNumber, startDate, endDate) {
    ...
    // Extended wait for results to render: networkidle + timeout + conditional wait for results indicators
    await page.waitForLoadState('networkidle', { timeout: 60000 }).catch(() => console.log('Network idle timeout after search, continuing.'));
    await page.waitForTimeout(this.headless ? 10000 : 7000); // Longer in headless to match debug-like timing, adjusted based on user feedback

    // Conditional wait: for either no-results text or specific results table (refined selector to avoid too many matches)
    try {
      await page.waitForSelector('table[role="grid"], div:has-text("Confirmation Number")', { state: 'visible', timeout: 30000 });
      // Further refine to the main results table
      await page.locator('table:has-text("Confirmation Number")').first().waitFor({ state: 'visible', timeout: 30000 });
    } catch (e) {
      console.warn(`Results table not detected for ${roomNumber} after extended wait, checking for no results: ${e.message}`);
    }
    if (this.debug || this.headless) await this.takeScreenshot(`after_search_results_render_${roomNumber}`); // Screenshot in both modes for diagnosis
      ...
      for (const room of batch) {
        ...
        while (attempts < 3) {
          try {
            ...
            if (attempts < 3) await this.page.waitForTimeout(5000); // Fixed: use this.page
          }
        }
        if (attempts === 3) this.failedRooms.push(room);
      }
      await this.page.waitForTimeout(3000); // Batch delay, fixed to this.page
    }
