// extractor.js - Increased delay after fill search to 2000ms before click Search; added --switch to constructor; processRooms limits to 1 room if switchMode; increased verification timeout
...
class OperaExtractor {
  ...
  async switchHotel(hotelName, hotelCode) {
    ...
    await page.waitForTimeout(2000); // Increased delay after fill
    const searchButton = page.getByRole('button', { name: 'Search', exact: true }).nth(1);
    ...
    await page.waitForTimeout(15000); // Increased for detection
    if (await hotelLocator.isVisible({ timeout: 15000 }).catch(() => false)) {
