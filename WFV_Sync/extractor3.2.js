// extractor.js - Updated switchHotel: click radio in table row for hotel selection (from screenshot), refined verification locator to top-right header text
...
class OperaExtractor {
  ...
  async switchHotel(hotelName, hotelCode) {
    ...
    // Verify with more flexible locator for header
    const hotelLocator = page.locator(':has-text("' + correctHotelText + '")');
    ...
    // Select radio in row with hotelName (from screenshot table structure)
    const hotelRow = page.locator('tr:has-text("' + hotelName + '")');
