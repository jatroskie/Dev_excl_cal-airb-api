// extractor.js - Added checkAndSwitchHotel method after initialize, using locators from provided script; if not on "Waterfront Village", switch; verify after switch
...
class OperaExtractor {
  ...
  async initialize() {
    ...
    await this.checkAndSwitchHotel(); // New: Check and switch if needed
  }

  async checkAndSwitchHotel() {
    const page = this.page;
    const correctHotelText = 'WFV - Waterfront Village'; // From screenshot
    const hotelLocator = page.getByText(correctHotelText);
    if (await hotelLocator.isVisible({ timeout: 10000 }).catch(() => false)) {
      console.log('Already on correct hotel: Waterfront Village.');
      return;
    }

    console.log('Switching to Waterfront Village...');
    ...
    // Select Waterfront Village
    const wfvOption = page.getByText('Waterfront Village');
    await wfvOption.waitFor({ state: 'visible', timeout: 30000 });
    await wfvOption.click();
    ...
    if (await hotelLocator.isVisible({ timeout: 10000 }).catch(() => false)) {
      console.log('Successfully switched to Waterfront Village.');
    } else {
      throw new Error('Failed to switch to Waterfront Village.');
    }
  }

  async downloadReservationCSV(roomNumber, startDate, endDate, slowMode = false) {
    const page = this.page;
  ...
  async processRooms(roomFile, startDate, endDate, batchSize = 5) {
    ...
    for (let i = 0; i < rooms.length; i += batchSize) {
      ...
      for (const room of batch) {
        ...
        while (attempts < 3) {
          try {
            const path = await this.downloadReservationCSV(room, startDate, endDate, slowMode);
