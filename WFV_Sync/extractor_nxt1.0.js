// extractor.js - Updated verification locator to page.locator(`text="${correctHotelText}"`), removed exact, added delay before verification
...
class OperaExtractor {
  ...
  async switchHotel(hotelName, hotelCode) {
    ...
    const hotelLocator = page.locator(`text="${correctHotelText}"`);
    ...
    // Wait for results, select first radio
    await page.waitForTimeout(2000); // Short wait for results
    const firstRadio = page.locator('input[type="radio"]').first();
    await firstRadio.waitFor({ state: 'visible', timeout: 30000 });
    ...
    await page.waitForTimeout(4000); // Reduced from 5000
    // Additional delay for header update
