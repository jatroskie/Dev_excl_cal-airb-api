// extractor.js - Revised with conditional waits after search: explicitly wait for either no-results or results table/View Options to ensure full render before checking; added extra stabilization in headless mode
...
class OperaExtractor {
  ...
  async downloadReservationCSV(roomNumber, startDate, endDate) {
    ...
    // Conditional wait: for either no-results text or results table/View Options to confirm full render
    try {
      await page.waitForSelector('text="You have no search results yet.", table, [role="link"][name="View Options"]', { state: 'visible', timeout: 30000 });
    } catch (e) {
      console.warn(`No clear results indicator detected for ${roomNumber} after extended wait: ${e.message}`);
    }
