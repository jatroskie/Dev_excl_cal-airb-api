// extractor.js - Added logic to detect and recover from 404 error pages after a hotel switch.
...
class OperaExtractor {
  ...
  async switchHotel(hotelName, hotelCode) {
    ...
    // Scope all subsequent actions to the "Select Location" dialog for robustness
    ...
    // Wait for results and click the hotel name to select it.
    ...
    // Wait for the dialog to close and then verify the switch by checking the header.
    await dialogLocator.waitFor({ state: 'hidden', timeout: 15000 });
    try {
      await hotelLocator.waitFor({ state: 'visible', timeout: 45000 }); // Wait for page to reload with new header
      console.log(`Successfully switched to ${hotelName}.`);
    } catch (e) {
      if (this.debug) await this.takeScreenshot(`verification_fail_${hotelName}`);
      throw new Error(`Failed to switch to ${hotelName}. Verification timed out.`);
    }
  }
  ...
  async downloadReservationCSV(roomNumber, startDate, endDate, slowMode = false, property = '') {
    ...
    // Check for and recover from a 404 error page, which can occur after a hotel switch.
    ...
    // After potential recovery, check if we are on the search form.
    // If not, we are probably on the results page, so click "Modify Search Criteria".
    ...
    if (!await arrivalFromInput.isVisible({ timeout: 5000 }).catch(() => false)) {
        ...
        try {
            ...
            // If we still can't find the form or the modify button, we're lost.
            // Navigate from scratch as a last resort.
  ...
  async navigateToManageReservation(page) {
    try {
      ...
      console.log('UI menu navigation failed. Attempting direct URL navigation (this is normal after a 404 error).');
  ...
  async processRooms(roomFile, startDate, endDate, batchSize = 5, property = '') {
    ...
    if (this.debug || this.switchMode) {
      rooms = rooms.slice(0, 1); // Process only one room in switch test or debug
    }
