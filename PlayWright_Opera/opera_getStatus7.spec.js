...
test.describe('Check All Rooms for March 7, 2025', () => {
  test('check room diary for all rooms', async ({ page, context }) => {
    ...
    for (const room of rooms) {
      ...
      // Check for occupant with refined locator
      const reservation = page.locator('//td[starts-with(text(), "CI |") or starts-with(text(), "IH |")]').first();
      try {
      ...
      // Modify search criteria with fallback
      const modifyLink = page.getBy