...
import { RoomAvailabilityTracker, createAvailabilityUI } from './room-availability';
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // New iCal configuration section
  const airbnbIcalUrl = 'YOUR_AIRBNB_MAIN_CALENDAR_ICAL_URL'; // Replace with your Airbnb iCal URL
  const icalsToRooms = {
    // Map your Airbnb property iCal URLs to room IDs
    // Format: 'ical_url': 'room_id'
    // Example: 'https://www.airbnb.com/calendar/ical/12345.ics': 'Room101'
  };
  ...
  // Function to process iCal data into FullCalendar events
  async function processIcalData(roomId) {
    try {
      // We'll need to use a proxy or server-side function to avoid CORS issues
      const response = await fetch(`/proxy-ical?url=${encodeURIComponent(airbnbIcalUrl)}`);
      const icalData = await response.text();
      
      // Process iCal data into events
      const events = [];
      // This will be handled by the iCalendar plugin automatically
      
      return events;
    } catch (error) {
      console.error(`Error fetching iCal data for room ${roomId}:`, error);
      showStatus(`Error loading Airbnb calendar for ${roomId}`, 'error');
      return [];
    }
  }
  ...
  try {
    calendar = new Calendar(calendarEl, {
      ...
      plugins: [resourceTimelinePlugin, iCalendarPlugin], // Add iCalendar plugin
      ...
      resources: [],
      events: [], // We'll populate this with both JSON and iCal data
      ...
      eventDidMount: function(info) {
        ...
        // Set special styling for Airbnb events
        if (info.event.extendedProps.source === 'airbnb') {
          info.el.style.backgroundColor = '#FF5A5F'; // Airbnb red
      ...
      // Add iCal events as a dynamic event source
      eventSources: [
        {
          // This is the main iCal feed for all properties
          url: `/proxy-ical?url=${encodeURIComponent(airbnbIcalUrl)}`,
          format: 'ics',
          failure: function() {
            showStatus('Error loading Airbnb calendar data', 'error');
          },
          // Process the iCal events to associate them with rooms and customize display
          eventDataTransform: function(eventData) {
            // Add Airbnb source marker
            ...
            // Map iCal URL to room ID if possible
            if (eventData.url && icalsToRooms[eventData.url]) {
              eventData.resourceId = icalsToRooms[eventData.url];
            }
    ...
    fetch('calendar-data.json')
      ...
      .then(data => {
        ...
        // Add existing events from your JSON data 
        // (will be merged with iCal events automatically)
        ...
        window.availabilityTracker = new RoomAvailabilityTracker(data);
        
        // Create the availability UI
        createAvailabilityUI(calendar, window.availabilityTracker);
  ...
  function showTooltip(event, element) {
    ...
    // Create tooltip content based on the event source
    let tooltipContent = `<h3>${event.title}</h3>`;
    
    if (props.source === 'airbnb') {
      // Airbnb-specific tooltip content
      tooltipContent += `
        <p><strong>Room:</strong> ${roomId}</p>
        <p><strong>Check-in:</strong> ${formatDisplayDate(event.start)}</p>
        <p><strong>Check-out:</strong> ${formatDisplayDate(event.end)}</p>
        <p><strong>Source:</strong> Airbnb</p>
      `;
    } else {
      // Default tooltip content for regular events
      tooltipContent += `
        <p><strong>Room:</strong> ${roomId}</p>
        <p><strong>Check-in:</strong> ${formatDisplayDate(event.start)}</p>
        <p><strong>Check-out:</strong> ${formatDisplayDate(event.end)}</p>
  ...
  function addImprovedStyles() {
    ...
    style.textContent = `
      ...
      /* Special styling for Airbnb bookings */
      .airbnb-event {
      ...
      /* Highlight available resources in the availability panel */
      .highlight-resource {
