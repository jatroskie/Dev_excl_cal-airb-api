// Description: FullCalendar Resource Timeline with iCalendar Plugin
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Configuration for individual property iCal feeds
  const roomIcalMappings = {
    // This will be populated from localStorage or the API
    // Format: 'roomId': 'ical_url'
    // Example: 'Room101': 'https://www.airbnb.com/calendar/ical/12345.ics'
  };
  ...
  // Function to load iCal mappings and create event sources
  function loadIcalMappings() {
    // Load room-to-iCal mappings from API
    fetch('/api/room-ical-mappings')
      ...
      .then(mappings => {
        ...
        // Create event sources for each room with an iCal URL
        Object.entries(mappings).forEach(([roomId, icalUrl]) => {
          if (!icalUrl) return;
          
          console.log(`Adding iCal feed for room ${roomId}: ${icalUrl}`);
          
          // Add this room's iCal feed as an event source
          calendar.addEventSource({
            id: `ical-${roomId}`,
            url: `/proxy-ical?url=${encodeURIComponent(icalUrl)}`,
            format: 'ics',
            resourceId: roomId, // Associate events with this specific room
            failure: function() {
              console.error(`Failed to load iCal data for room ${roomId}`);
            },
        ...
        // Refetch events to display the iCal data
        calendar.refetchEvents();
        
        showStatus(`Loaded ${Object.keys(mappings).length} Airbnb calendar feeds`, 'success');
      })
      .catch(error => {
        console.error('Error loading iCal mappings:', error);
        showStatus('Error loading Airbnb calendar mappings', 'error');
      });
  }
  ...
  try {
    calendar = new Calendar(calendarEl, {
      ...
      // We'll add individual iCal sources for each room programmatically
      eventSources: []
    });
    ...
    fetch('calendar-data.json')
      ...
      .then(data => {
        ...
        // Load iCal mappings and add event sources for each room
        loadIcalMappings();
      })
      .catch(error => {
