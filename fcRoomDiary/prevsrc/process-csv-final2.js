...
async function getICalUrls() {
  try {
    ...
    const resourcesPath = path.join(__dirname, 'fixed-resources.json');
    
    // Load and parse the resources file
    ...
    // Return empty map if there's an error, so script can continue
    return {};
  }
}
...
async function processAndMergeCSVs() {
  ...
  for (const roomId in roomMapping) {
    if (iCalMap[roomId]) {
      ...
      console.log(`Added iCal URL for room ${roomId}`);
    } else {
...
async function main() {
  try {
    ...
    await writeRoomMappingToFile(); // Optional: Write the updated roomMapping to a file
  } catch (error) {
