...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function updateAvailabilityButtonText() {
    ...
    if (button) {
      button.textContent = showingAvailableOnly 
        ...
        : `Show Available Rooms (${availableRooms.length})`;
  ...
  function applyResponsiveStyles() {
    ...
    // Style the Available Rooms button
    const availableButton = document.querySelector('.fc-availableRooms-button');
    if (availableButton) {
      if (showingAvailableOnly) {
        availableButton.classList.add('fc-button-active');
      } else {
        availableButton.classList.remove('fc-button-active');
      }
    }
  }
});