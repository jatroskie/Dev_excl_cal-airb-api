// Description: This script processes CSV files from a downloads folder and merges them with existing JSON data.
// It extracts resources and events from the CSV files and merges them with existing data.
// The script assumes that the CSV files contain reservation data for rooms and that the file names contain the room numbers.
// The script also assumes that the CSV files are in a specific format and that the data is clean.
// The script outputs a JSON file with the merged resources and events.
// The script uses PapaParse for parsing CSV data and assumes that the Papa global is available.
// The script uses fs.promises for file operations and assumes that the fs global is available.
// The script uses path for file path operations and assumes that the path global is available.
// The script uses a custom formatDate function to convert date strings to ISO format.
// The script uses a custom getReservationClasses function to determine reservation classes based on status.
// The script uses a custom mergeCalendarData function to merge resources and events with existing data.
// The script uses a custom removeDuplicateResources function to remove duplicate resources.
// The script is designed to be run in a Node.js environment.
// The script is intended to be run manually or as part of a larger automation process.
// run the script using node csv-processMerge2.js using parameters: downloadsDirectory, outputDirectory
// Next step: to ???
...
async function processAndMergeCSVs() {
  ...
  const downloadsDirectory = 'C:\\Users\\jatro\\Dev\\OperaLoginAppLauncher\\downloads';
