...
# Step 3: Create view for calendar bookings
cursor.execute("""
...
# Step 4: Query for availability per day for the next 7 days
current_date = datetime(2025, 3, 5)  # Current date: March 5, 2025
dates = [(current_date + timedelta(days=i)).strftime("%Y-%m-%d") for i in range(7)]
date_range = [current_date.strftime("%Y-%m-%d"), (current_date + timedelta(days=6)).strftime("%Y-%m-%d")]

print(f"\nRoom Availability for Each Day (March {dates[0]} - {dates[-1]}):")
print("Room\tRoom Type\t" + "\t".join([d.split("-")[-1] for d in dates]))
print("-" * 80)

# Get all unique rooms and their types
cursor.execute("SELECT DISTINCT room, room_type FROM bookings ORDER BY room ASC")
all_rooms = cursor.fetchall()

for room, room_type in all_rooms:
    status_per_day = []
    for date in dates:
        cursor.execute("""
            SELECT COUNT(*) 
            ...
            AND departure > ?
        """, (room, date, date))
        is_booked = cursor.fetchone()[0] > 0
        status_per_day.append("Booked" if is_booked else "Available")
    print(f"{room}\t{room_type}\t" + "\t".join(status_per_day))

# Step 5: Fetch data from the calendar_bookings view for visualization
cursor.execute("SELECT room, arrival, departure, name FROM calendar_bookings")
bookings = cursor.fetchall()

# Step 6: Visualize bookings with color-coding
...
print("SQLite database created, data loaded, availability queried, and calendar visualized!")