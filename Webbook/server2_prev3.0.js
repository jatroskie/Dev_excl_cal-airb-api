...
// Corrected CALLBACK_SECRET2 to CALLBACK_SECRET to match usage
...
app.post('/resLoader', async (req, res) => {
    ...
    // Regex to validate various complex room ID formats based on provided examples.
    // Covers patterns like:
    // 1. XXXX-NNNN (e.g., TTBH-6161)
    // 2. XXX-Alphanumeric{1,8} (e.g., CRV-8366, HES-IC062, TQA-SALDW, LAW-361A)
    // 3. X...LUX-N or XX...SUP-N (e.g., D581SUP-2, PA1S41PEN-3)
    // 4. XXX-N-AAAAA (e.g., CRV-6-AMAND)
    // 5. XNNNAAAAAN (no hyphen) (e.g., B601PPENT3)
    ...
    const sanitizedInputs = {
        ...
        roomId: String(roomId).replace(/[^A-Z0-9-]/g, ''), // Sanitization remains consistent with allowed chars
...
app.listen(PORT, () => {
    ...
    if (!CALLBACK_URL || !CALLBACK_SECRET) {
        ...
        logger.info(`Callback Secret is ${CALLBACK_SECRET ? 'SET' : 'NOT SET'}`); // Matches corrected console log above
    }
});