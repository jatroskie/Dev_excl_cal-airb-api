...
function RateEditor() {
  ...
  const handleExecutePaste = async (config) => {
    ...
    const sourceYear = getYear(sourceData.seasons[0].startDate.toDate());
    const yearDiff = targetYear - sourceYear;
    const firstNewDate = addDays(sourceData.seasons[0].startDate.toDate(), yearDiff * 365.25);
    const lastNewDate = addDays(sourceData.seasons[sourceData.seasons.length - 1].endDate.toDate(), yearDiff * 365.25);
    ...
    try {
      ...
      sourceData.seasons.forEach(season => {
        ...
        const currentSourceYear = getYear(season.startDate.toDate());
        const currentYearDiff = targetYear - currentSourceYear;

        const newStartDate = addDays(season.startDate.toDate(), currentYearDiff * 365.25);
        const newEndDate = addDays(season.endDate.toDate(), currentYearDiff * 365.25);
        ...
        const newSeasonData = { ...season, unitType: targetUnitType, startDate: Timestamp.fromDate(newStartDate), endDate: Timestamp.fromDate(newEndDate), weekdayRateAgent: newWeekdayRate, weekendRateAgent: newWeekendRate, seasonName: updateNames ? season.seasonName.replace(/\d{4}/g, (match) => parseInt(match) + currentYearDiff) : season.seasonName, lastUpdatedAt: Timestamp.now() };
