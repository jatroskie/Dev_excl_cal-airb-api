...
function RateEditor() {
  ...
  const [pastingConfig, setPastingConfig] = useState(null); // Holds data for the paste panel
  ...
  const handleCopyYear = (unitType, year) => {
    ...
    if (seasonsToCopy.length === 0) {
      alert("No seasons found in this year to copy.");
      return;
    }
    setClipboard({
      mode: 'fullYear',
    ...
    // Optional: Add a toast notification here for better UX
  };

  const handleInitiatePaste = (targetUnitType) => {
    if (!clipboard) {
      alert("Clipboard is empty. Please copy a year's rates first.");
      return;
    }
  ...
  const handleExecutePaste = async (config) => {
    ...
    // 1. Safety Check: Find seasons to be overwritten
    ...
    if (seasonsInTargetYear.length > 0 && !allowOverwrite) {
      alert("Error: Target year contains existing seasons and overwrite was not confirmed.");
      return;
    }
    ...
    try {
      ...
      // 2. Handle Overwrites: Delete existing seasons if confirmed
      if (seasonsInTargetYear.length > 0 && allowOverwrite) {
      ...
      // 3. Loop, Transform, and Add New Seasons to Batch
      ...
      sourceData.seasons.forEach(season => {
        ...
        // Construct new season data
        const newSeasonData = {
          ...season, // copy base fields like currency, minStayNights etc.
          id: newSeasonRef.id, // remove original id
          unitType: targetUnitType,
          startDate: Timestamp.fromDate(newStartDate),
          endDate: Timestamp.fromDate(newEndDate),
          weekdayRateAgent: newWeekdayRate,
          weekendRateAgent: newWeekendRate,
          seasonName: updateNames 
            ? season.seasonName.replace(sourceData.sourceYear, targetYear) 
            : season.seasonName,
          lastUpdatedAt: Timestamp.now(),
        };
        delete newSeasonData.id; // Firestore generates its own ID
      ...
      setClipboard(null);
      setStartDisplayDate(new Date(startDisplayDate)); // Trigger refresh
      // Optional: Add a success toast notification here

    } catch (err) {
      console.error("Failed to execute paste operation:", err);
      alert("A critical error occurred during the paste operation. Please check the console.");
      setPastingConfig(null); // Close panel on error
    }
  };
  ...
  return (
    <div className="rate-editor-container">
      ...
      {pastingConfig && (<PasteConfigPanel clipboard={pastingConfig.clipboard} targetUnitType={pastingConfig.targetUnitType}
          allRates={rates} // Pass all rates for the overwrite check
