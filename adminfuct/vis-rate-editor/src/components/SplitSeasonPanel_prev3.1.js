...
function SplitSeasonPanel({ originalSeason, onSave, onClose }) {
  ...
  const handleSplit = async (e) => {
    ...
    if (isOverlap(parsedSplitDate, originalSeason.endDate.toDate(), allSeasonsForUnit, originalSeason.id)) {
        toast.error("The new season created by this split would overlap with an existing season.");
        return;
    }
    setIsSaving(true);
    try {
