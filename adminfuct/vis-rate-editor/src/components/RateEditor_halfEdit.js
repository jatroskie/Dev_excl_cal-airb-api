...
// Cleaned up unused date-fns imports
import { startOfMonth, addMonths, getYear, getMonth, addDays, format } from 'date-fns'; 
...
function RateEditor() {
  ...
  const seasonsToUpdate = rates.filter(r => selectedSeasons.has(r.id));
    seasonsToUpdate.forEach(season => {
      ...
      if (adjustmentType === 'percentage') {
        if (weekdayValue !== 0) newWeekdayRate *= (1 + weekdayValue / 100);
        if (weekendValue !== 0) newWeekendRate *= (1 + weekendValue / 100);
      } else { // fixed_amount
        if (weekdayValue !== 0) newWeekdayRate += weekdayValue;
        if (weekendValue !== 0) newWeekendRate += weekendValue;
    }
    batch.update(seasonRef, {
      weekdayRateAgent: Math.round(newWeekdayRate * 100) / 100,
      weekendRateAgent: Math.round(newWeekendRate * 100) / 100,
      minStay: minStay !== undefined ? minStay : season.minStay,
  ...
  const handleAddSeason = (unitType) => { 
    setAddingSeasonFor(unitType);
  };
  const handleNewSeasonSave = () => { 
    setAddingSeasonFor(null); 
    setStartDisplayDate(new Date(startDisplayDate));
  };
  ...
  if (minStay !== null) {
        dataToUpdate.minStayNights = minStay;
      }
      
      batch.update(seasonRef, dataToUpdate);
    });

    try {
      await batch.commit();
      toast.success(`Successfully updated ${selectedSeasons.size} seasons!`);
      setSelectedSeasons(new Set()); // Clear selection
      setStartDisplayDate(new Date(startDisplayDate)); // Refresh data
    } catch (err) {
  ...
  const handleOverrideUpdate = () => {
    ...
    setStartDisplayDate(new Date(startDisplayDate)); // Trigger a refresh
  };
  ...
  const handleBoundaryDrag = async (leftSeason, rightSeason, daysMoved) => {
    ...
    if (newBoundary <= leftSeasonStartDate || newBoundary >= rightSeasonEndDate) { toast.error("Invalid drag."); return; }
  ...
  // ====================================================================
  // CHANGE START: Updated Copy/Paste logic
  // ====================================================================
  const handleCopySeasons = (seasonsToCopy, unitType, modeLabel) => {
    if (!seasonsToCopy || seasonsToCopy.length === 0) {
      toast.error("No seasons found to copy for the selected action.");
      return;
    }
    setClipboard({
      mode: 'custom',
      ...
      seasons: seasonsToCopy,
    });
  };

  const handleInitiatePaste = (targetUnitType) => {
    if (!clipboard) {
      toast.error("Clipboard is empty. Please perform a copy action first.");
      return;
    }
  ...
  // ====================================================================
  // CHANGE START: Updated Paste Logic
  // ====================================================================
 const handleExecutePaste = async (config) => {
    ...
    // 1. Find the actual end date of the last season IN THE DATABASE to shift from
    const allTargetSeasons = rates.filter(r => r.unitType === sourceData.sourceUnitType).sort((a,b) => a.startDate.toMillis() - b.startDate.toMillis());
    const firstOriginalSeason = sourceData.seasons[0];
    const firstSeasonIndex = allTargetSeasons.findIndex(s => s.id === firstOriginalSeason.id);
    ...
    if (shiftFromIndex >= allTargetSeasons.length || shiftFromIndex < 0) {
       toast.error(`Error: Cannot shift by ${shiftBySeasons} seasons. Not enough subsequent seasons exist.`);
        return;
    }

    const shiftFromDate = allTargetSeasons[shiftFromIndex].endDate.toDate();

    // 2. Calculate the start/end dates of the NEW block of seasons
    ...
    // 3. Safety Check for Overlaps (This logic remains the same)
    const seasonsInTargetRange = rates.filter(r => {
      if (r.unitType !== targetUnitType) return false;
      const rStart = r.startDate.toDate();
      const rEnd = r.endDate.toDate();
      return rStart < lastNewDate && rEnd > firstNewDate;
    });

    if (seasonsInTargetRange.length > 0 && !allowOverwrite) {
      toast.error("Error: The target date range contains existing seasons, and overwrite was not confirmed.");
      return;
    }

    // 4. Loop, Transform, and Paste (This logic is now simpler)
    try {
      const batch = writeBatch(db);
      if (seasonsInTargetRange.length > 0 && allowOverwrite) {
        seasonsInTargetRange.forEach(season => {
          batch.delete(doc(db, "apartmentSeasonRates", season.id));
        });
      }
      ...
      sourceData.seasons.forEach(season => {
        ...
        const newName = updateNames 
          ? season.seasonName.replace(/\d{4}/g, (match) => getYear(newStartDate))
          : season.seasonName;
    ...
    <div className="rate-editor-container">
      ...
      {clipboard && (
        <div className="clipboard-info">
          Copied: {clipboard.sourceUnitType} ({clipboard.modeLabel}). Right-click a row to paste.
