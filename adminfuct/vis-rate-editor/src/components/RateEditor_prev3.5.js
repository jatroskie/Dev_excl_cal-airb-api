...
function RateEditor() {
  ...
  const handleCopySeasons = (seasonsToCopy, unitType, modeLabel) => {
    if (!seasonsToCopy || seasonsToCopy.length === 0) {
      alert("No seasons found to copy for the selected action.");
      return;
    }
    setClipboard({
      mode: 'custom',
  ...
  const handleInitiatePaste = (targetUnitType) => {
    if (!clipboard) {
      alert("Clipboard is empty. Please perform a copy action first.");
      return;
    }
 ...
 const handleExecutePaste = async (config) => {
    ...
    if (firstSeasonIndex === -1) {
        alert("Error: Could not find the source season in the database to calculate shift.");
        return;
    }
    
    const shiftFromIndex = firstSeasonIndex + shiftBySeasons - 1;
    if (shiftFromIndex >= allTargetSeasons.length || shiftFromIndex < 0) {
        alert(`Error: Cannot shift by ${shiftBySeasons} seasons. Not enough subsequent seasons exist.`);
        return;
    }
    ...
    if (seasonsInTargetRange.length > 0 && !allowOverwrite) {
      alert("Error: The target date range contains existing seasons, and overwrite was not confirmed.");
      return;
    }
