...
import {
  ...
  max,
  min,
  getYear, // <-- THE FIX: Added getYear to the import list
} from 'date-fns';
...
// --- NEW COMPONENT: The Context Menu ---
...
function RateTimeline({ unitTypes, ratesByUnitType, overridesByUnitType, viewStartDate, onSelectSeason, onBoundaryDrag, clipboard, onCopyYear, onInitiatePaste }) {
  ...
  const handleContextMenu = (e, unitType) => {
    e.preventDefault();
    const currentYear = getYear(viewStartDate); // This will now work correctly
    const options = [
      { label: `Copy Full Year (${currentYear})`, action: () => { onCopyYear(unitType, currentYear); setContextMenu(null); } }
    ];

    if (clipboard) {
      options.push({ label: `Paste '${clipboard.sourceUnitType}' here`, action: () => { onInitiatePaste(unitType); setContextMenu(null); } });
    }
