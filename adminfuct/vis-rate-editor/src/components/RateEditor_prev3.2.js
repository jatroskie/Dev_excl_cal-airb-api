...
function RateEditor() {
  ...
  const handleCopyYear = (unitType, year) => {
    const seasonsToCopy = rates.filter(r => {
      if (r.unitType !== unitType) return false;
      // THE NEW LOGIC: Copy if the season STARTS in the year OR ENDS in the year.
      // This correctly includes seasons that cross the Dec/Jan boundary.
      return getYear(r.startDate.toDate()) === year || getYear(r.endDate.toDate()) === year;
    });
