...
import BulkEditPanel from './BulkEditPanel'; // Import the new panel
...
function RateEditor() {
  ...
  useEffect(() => {
    if (!selectedHotel) { setLoading(false); return; }
  ...
  const handleExecutePaste = async (config) => {
    ...
    if (firstSeasonIndex === -1) { toast.error("Error: Could not find source season to calculate shift."); return; }
    const shiftFromIndex = firstSeasonIndex + shiftBySeasons - 1;
    if (shiftFromIndex >= allTargetSeasons.length || shiftFromIndex < 0) { toast.error(`Error: Cannot shift by ${shiftBySeasons} seasons.`); return; }
    ...
    if (seasonsInTargetRange.length > 0 && !allowOverwrite) { toast.error("Error: Overwrite not confirmed."); return; }
    try {
      const batch = writeBatch(db);
      ...
      sourceData.seasons.forEach(season => {
        ...
        const newName = updateNames ? season.seasonName.replace(/\d{4}/g, () => getYear(newStartDate)) : season.seasonName;
        const newSeasonData = { ...season, unitType: targetUnitType, startDate: Timestamp.fromDate(newStartDate), endDate: Timestamp.fromDate(newEndDate), weekdayRateAgent: newWeekdayRate, weekendRateAgent: newWeekendRate, seasonName: newName, lastUpdatedAt: Timestamp.now() };
