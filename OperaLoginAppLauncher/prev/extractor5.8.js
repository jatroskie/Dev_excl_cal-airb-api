...
class OperaCloudExtractor {
  ...
  async navigateToManageReservation(page) {
    try {
      ...
      await bookingsMenuItem.waitFor({ state: 'visible', timeout: 30000 }).catch(async (e) => {
        console.log('Timeout waiting for Bookings menu, trying to navigate directly to dashboard...');
      ...
      console.log('Attempting direct navigation after error...');
      // Try a direct navigation approach if menu navigation fails
      await page.goto('https://mtce4.oraclehospitality.eu-frankfurt-1.ocs.oraclecloud.com/OPERA9/opera/operacloud/faces/pages/reservation/manageReservation', { 
        waitUntil: 'domcontentloaded',
        timeout: 60000 
    ...
    try {
      ...
      try {
        ...
        if (currentUrl.includes('manageReservation')) {
          console.log('Already on Manage Reservation screen, navigating to main interface...');
          await activePage.goto('https://mtce4.oraclehospitality.eu-frankfurt-1.ocs.oraclecloud.com/OPERA9/opera/operacloud/faces/opera-cloud-index/OperaCloud', { 
            waitUntil: 'domcontentloaded',
            timeout: 60000 
          });
          ...
          console.log('Navigated to main interface from reservation page');
        }
        
        // Check for the Bookings menu
        const bookingsMenuItem = activePage.locator('#pt1\\:toggle');
