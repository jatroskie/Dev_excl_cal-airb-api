...
class OperaCloudExtractor {
  ...
  async downloadReservationCSV(roomNumber, startDate, endDate, attempt = 1) {
    try {
     ...
     // await page.getByRole('textbox', { name: 'Room', exact: true }).fill(roomNumber)
      ...
      // Add a small wait to ensure dropdown is fully loaded
      await this.page.waitForTimeout(500);

      // Click on Export
     // await this.page.getByText('Export').click();
      const exportOption = this.page.locator('a').filter({ hasText: 'Export' }).locator('span');
      await exportOption.waitFor({ state: 'visible', timeout: 5000 });
      await exportOption.click();
      console.log('Export on view options clicked...');

      // Add a small wait to ensure submenu is fully loaded
      await this.page.waitForTimeout(500);

      // Click on CSV
      await this.page.getByText('CSV').click();
      console.log('CSV file output selected...');
      // Wait for the File Name textbox to be visible before interacting with it
      await this.page.getByRole('textbox', { name: 'File Name' }).waitFor({state: 'visible', timeout: 5000});
