...
// Ensure no modal overlay is blocking interaction
...
// Robust selector for "New Profile" link (updated to use hasText)
const newProfileLink = page.locator('a').filter({ hasText: /New Profile/i }).first();

// Log link details for debugging
const linkDetails = await newProfileLink.evaluate(el => ({
  text: el.textContent?.trim() || '',
...
console.log('New Profile link details:', linkDetails || 'Not found');

// Check visibility and click
let newProfileClicked = false;
if (await newProfileLink.isVisible({ timeout: 10000 })) {
  console.log('New Profile link found on main page, clicking...');
  await newProfileLink.scrollIntoViewIfNeeded();
  await newProfileLink.click();
  newProfileClicked = true;
} else {
  console.log('New Profile link not found on main page, checking iframes...');
  ...
  for (const iframeSelector of iframeSelectors) {
    ...
    const iframeNewProfileLink = iframe.locator('a').filter({ hasText: /New Profile/i }).first();
    if (await iframeNewProfileLink.isVisible({ timeout: 3000 })) {
      console.log(`New Profile link found in iframe (${iframeSelector}), clicking...`);
      await iframeNewProfileLink.scrollIntoViewIfNeeded();
      await iframeNewProfileLink.click();
      newProfileClicked = true;
      break;
    }
  }
...
if (!newProfileClicked) {
  console.log('New Profile link not found, using JavaScript fallback...');
  const clicked = await page.evaluate(() => {
    const links = Array.from(document.querySelectorAll('a'))
      .filter(el => el.textContent?.trim().toLowerCase() === 'new profile' && el.offsetParent !== null);
    if (links.length > 0) {
  ...
  if (clicked) {
    ...
    throw new Error('Could not find or click New Profile link with any strategy');
  }
}

// Wait for the Guest Profile dialog with improved stability
console.log('Waiting for Guest Profile dialog to appear...');
await page.waitForTimeout(5000); // Initial wait for dialog to load

let profileDialog = null;
let dialogFrame = null; // To track if we're in an iframe

// Debug log to ensure we reach this point
console.log('Starting dialog location logic...');

try {
  // Try main page first
  const dialogStrategies = [
    // Strategy 1: Role-based
    { locator: page.getByRole('dialog', { name: /Guest Profile|Profile|Create Profile/i }), desc: 'role-based' },
    // Strategy 2: Generic div with relevant attributes
    { locator: page.locator('div[aria-label*="Profile" i], div[id*="profile" i], div[aria-label*="Guest" i]'), desc: 'attribute-based' },
    // Strategy 3: Text-based
    { locator: page.locator('div').filter({ has: page.locator('text=/Guest Profile|Profile|Create Profile/i') }), desc: 'text-based' }
  ];

  for (const strategy of dialogStrategies) {
    if (await strategy.locator.isVisible({ timeout: 5000 })) {
      profileDialog = strategy.locator;
      dialogFrame = page; // Main page context
      console.log(`Guest Profile dialog found using ${strategy.desc} strategy on main page`);
      break;
    }
  }
  ...
  if (!profileDialog) {
    ...
    const iframeSelectors = ['iframe[title="Content"]', 'iframe[id*="popup"]', 'iframe[id*="dialog"]', 'iframe'];
    for (const iframeSelector of iframeSelectors) {
      const iframe = page.frameLocator(iframeSelector);
      for (const strategy of dialogStrategies) {
        const iframeDialog = strategy.locator.call(iframe); // Apply strategy within iframe
        if (await iframeDialog.isVisible({ timeout: 3000 })) {
          profileDialog = iframeDialog;
          dialogFrame = iframe; // Set iframe context
          console.log(`Guest Profile dialog found using ${strategy.desc} strategy in iframe (${iframeSelector})`);
          break;
        }
      }
      if (profileDialog) break;
    }
  }
  ...
  console.error('Error while locating Guest Profile dialog:', error.message);
  throw error; // Re-throw to fail the test
}

// Debug log to confirm we exit the try-catch block
console.log('Finished dialog location logic.');

await page.screenshot({ path: path.join(downloadsPath, 'guest_profile_dialog_opened.png') });
console.log('Guest Profile dialog found and visible, screenshot taken');

// Fill guest details
console.log('Filling in Last Name...');
const nameInput = dialogFrame.getByRole('textbox', { name: /Name|Last Name|Surname/i }).first();
...
const firstNameInput = dialogFrame.getByRole('textbox', { name: /First Name/i }).first();
...
const mobileInput = dialogFrame.getByRole('textbox', { name: /Mobile|Phone|Communication Value/i }).first();
if (await mobileInput.isVisible({ timeout: 5000 })) {
  await mobileInput.fill(telephoneNumber);
  console.log(`Set Mobile Number to: ${telephoneNumber}`);
} else {
  throw new Error('Mobile Number field not found');
}

console.log('Clicking Save and Select Profile button...');
const saveButton = dialogFrame.getByRole('button', { name: 'Save and Select Profile' }).first();
if (await saveButton.isVisible({ timeout: 5000 })) {
  await saveButton.click();
  console.log('Clicked Save and Select Profile button');
} else {
  throw new Error('Save and Select Profile button not found');
}

console.log('Waiting for dialog to close and UI to stabilize...');
await page.waitForTimeout(10000); // Extended wait for stability
await page.screenshot({ path: path.join(downloadsPath, 'after_profile_save.png') });
    //-------------------------------------------
    
    //-----+++++--------------------------------------
   // BUTTON ANALYSIS - Try to find any visible buttons on the page and list them
