...
async function createBooking(clientName, firstName, telephoneNumber, roomNumber, 
  ...
  try {
    ...
    const newProfileLink1 = page.getByRole('link', { name: 'New Profile' }).first();
    if (await newProfileLink1.isVisible({ timeout: 10000 })) {
      console.log('Found New Profile link with role-based locator');
      await newProfileLink1.scrollIntoViewIfNeeded();
      await newProfileLink1.click();
      console.log('Clicked New Profile link (Strategy 1)');
      newProfileClicked = true;
    } else {
      console.log('New Profile link not found with role-based locator');
    }
    ...
    if (newProfileClicked) {
      try {
        ...
        await profileDialog.waitFor({ state: 'visible', timeout: 15000 });
        await page.waitForTimeout(2000);
