...
class OperaCloudExtractor {
 ...
 // Update the initialize method to properly configure downloads
  async initialize() {
  try {
    ...
    // IMPORTANT: Configure the context to handle downloads properly
  ...
  async navigateToManageReservation(page) {
    try {
      ...
      // Look for Manage Reservation option - also target by similar structure
      console.log('Clicking Manage Reservation...');
      const manageReservationItem = page.locator('tr[role="menuitem"] td:has-text("Manage Reservation")').first();
      ...
      try {
        ...
        try {
          ...
          const reservationsText = page.locator('text="Reservations"').first();
          if (await reservationsText.isVisible({ timeout: 3000 })) {
            console.log('Found Reservations text, clicking...');
            await reservationsText.click();
            await page.waitForTimeout(2000);
            
            // Try to find any element containing "Manage Reservation" text
            const manageResText = page.locator('text="Manage Reservation"').first();
            if (await manageResText.isVisible({ timeout: 3000 })) {
              console.log('Found Manage Reservation text, clicking...');
              await manageResText.click();
              await page.waitForTimeout(3000);
        ...
        // Second alternative: Try direct URL navigation
        try {
  ...
  // Update the downloadReservationCSV method with improved download handling
