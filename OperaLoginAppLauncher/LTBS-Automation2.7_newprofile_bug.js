...
let dialogContext = page;
let dialogInIframeFound = false;

// First, check if the dialog is on the main page
const dialogOnMainPage = page.locator('div').filter({ has: page.locator('text=/Guest Profile|Profile|Create Profile/i') });
if (await dialogOnMainPage.isVisible({ timeout: 3000 })) {
  console.log('Dialog confirmed on main page, using page context.');
  dialogContext = page;
} else {
  console.log('Dialog not confirmed on main page, checking iframes...');
  const iframeSelectors = [
    'iframe[title="Content"]',
    'iframe[id*="popup"]',
    'iframe[id*="dialog"]'
  ...
  for (const iframeSelector of iframeSelectors) {
    ...
    if (await dialogInIframe.isVisible({ timeout: 3000 }).catch(() => false)) {
      dialogContext = iframeLocator;
      dialogInIframeFound = true;
      console.log(`Using iframe context: ${iframeSelector}`);
      break;
    }
  }

  if (!dialogInIframeFound) {
    console.log('Dialog not found with specific iframe selectors, iterating over all iframes...');
    const allIframes = await page.locator('iframe').all();
    for (let i = 0; i < allIframes.length; i++) {
      const iframe = allIframes[i];
      ...
      if (await dialogInIframe.isVisible({ timeout: 3000 }).catch(() => false)) {
        dialogContext = iframeLocator;
        dialogInIframeFound = true;
        console.log(`Using iframe context: iframe ${iframeName} (index ${i})`);
        break;
      }
    }
  }
  ...
  if (!dialogInIframeFound) {
    console.log('Dialog not found in any iframe, defaulting to page context.');
    dialogContext = page;
  }
}
