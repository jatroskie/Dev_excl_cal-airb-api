...
await page.waitForTimeout(5000); // Increased wait time to ensure dialog appears

// Function to check if the Guest Profile dialog is open
async function checkGuestProfileDialog() {
  let dialogContext = page;
  let dialogFound = false;
  let dialogLocator = page.locator('#pt1\\:oc_pg_pt\\:r1\\:2\\:ltbbk\\:oc_pnl_cmp\\:oc_scrn_pnl_tmpl\\:oc_scrn_tmpl_zm34g1\\:oc_scrn_pnl_pnl\\:oc_pnl_tmpl_zm34g1\\:oc_pnl_cntr');

  // Strategy 1: Check if the dialog is on the main page using the specific ID
  console.log('Trying to locate Guest Profile dialog on main page with specific ID...');
  let isVisibleOnMainPage = await dialogLocator.isVisible({ timeout: 15000 }).catch(() => false);
  if (isVisibleOnMainPage) {
    // Verify visibility properties using JavaScript
    const visibilityCheck = await page.evaluate(() => {
      const dialog = document.querySelector('#pt1\\:oc_pg_pt\\:r1\\:2\\:ltbbk\\:oc_pnl_cmp\\:oc_scrn_pnl_tmpl\\:oc_scrn_tmpl_zm34g1\\:oc_scrn_pnl_pnl\\:oc_pnl_tmpl_zm34g1\\:oc_pnl_cntr');
      ...
      return {
        ...
        textContent: dialog.textContent.trim().substring(0, 100)
      };
    });

    if (
      visibilityCheck &&
      visibilityCheck.isConnected &&
      visibilityCheck.display === 'block' &&
      visibilityCheck.visibility === 'visible' &&
      visibilityCheck.opacity === '1'
    ) {
      console.log('Guest Profile dialog confirmed open on main page with specific ID.');
      dialogFound = true;
      dialogContext = page;
    } else {
      console.log('Guest Profile dialog found but not visible on main page:', visibilityCheck);
    }
  }

  // Strategy 2: If not found on main page, check iframes
  if (!dialogFound) {
    console.log('Guest Profile dialog not found on main page, checking iframes...');
    ...
    for (const iframeSelector of iframeSelectors) {
      ...
      dialogLocator = iframeLocator.locator('#pt1\\:oc_pg_pt\\:r1\\:2\\:ltbbk\\:oc_pnl_cmp\\:oc_scrn_pnl_tmpl\\:oc_scrn_tmpl_zm34g1\\:oc_scrn_pnl_pnl\\:oc_pnl_tmpl_zm34g1\\:oc_pnl_cntr');
      const isVisibleInIframe = await dialogLocator.isVisible({ timeout: 3000 }).catch(() => false);
      if (isVisibleInIframe) {
        // Verify visibility properties in iframe
        const visibilityCheck = await iframeLocator.evaluate(() => {
          const dialog = document.querySelector('#pt1\\:oc_pg_pt\\:r1\\:2\\:ltbbk\\:oc_pnl_cmp\\:oc_scrn_pnl_tmpl\\:oc_scrn_tmpl_zm34g1\\:oc_scrn_pnl_pnl\\:oc_pnl_tmpl_zm34g1\\:oc_pnl_cntr');
          if (!dialog) return false;
        ...
        if (
          ...
          console.log(`Guest Profile dialog confirmed open in iframe with selector: ${iframeSelector}`);
          break;
        } else {
          console.log(`Guest Profile dialog found but not visible in iframe (${iframeSelector}):`, visibilityCheck);
        }
      }
    }
  }

  // Strategy 3: Iterate over all iframes explicitly if not found with specific selectors
  if (!dialogFound) {
    console.log('Dialog not found with specific iframe selectors, iterating over all iframes...');
    const allIframes = await page.locator('iframe').all();
    for (let i = 0; i < allIframes.length; i++) {
      const iframe = allIframes[i];
      const iframeName = await iframe.getAttribute('name') || `iframe-${i}`;
      if (iframeName.includes('afr::PushIframe') || iframeName.includes('content_list') || iframeName.includes('launcher')) {
        console.log(`Skipping irrelevant iframe: ${iframeName}`);
        continue;
      }
      ...
      if (isVisibleInIframe) {
        ...
        if (
          ...
          dialogFound = true;
          dialogContext = iframeLocator;
          console.log(`Guest Profile dialog confirmed open in iframe: ${iframeName} (index ${i})`);
          break;
        } else {
          console.log(`Guest Profile dialog found but not visible in iframe (${iframeName}):`, visibilityCheck);
        }
      }
    }
  }

  // Strategy 4: Fallback to JavaScript if still not found
  if (!dialogFound) {
    console.log('Guest Profile dialog not found with specific ID, trying JavaScript fallback...');
    ...
    if (dialogInfo.length > 0) {
      dialogLocator = page.locator(`div[id="${dialogInfo[0].id}"]`).first();
      const isVisible = await dialogLocator.isVisible({ timeout: 5000 }).catch(() => false);
      if (isVisible) {
        // Verify visibility properties
        const visibilityCheck = await page.evaluate((dialogId) => {
          const dialog = document.querySelector(`div[id="${dialogId}"]`);
          if (!dialog) return false;
        ...
        if (
          ...
          dialogFound = true;
          console.log('Guest Profile dialog confirmed open using JavaScript fallback');
        } else {
          console.log('Guest Profile dialog found via JavaScript but not visible:', visibilityCheck);
        }
      }
    }
  }
  ...
  if (!dialogFound) {
    ...
    // Log all visible divs for debugging
  ...
  // Take a screenshot to confirm the dialog is open
  await dialogLocator.scrollIntoViewIfNeeded();
  ...
  return { isOpen: true, dialogContext, dialogLocator };
}

// Check if the dialog is open
const { isOpen, dialogContext, dialogLocator } = await checkGuestProfileDialog();
if (!isOpen) {
  throw new Error('Guest Profile dialog did not open');
//}
//------------------------------------------------------------------
...
if (!newProfileClicked) {
 ...
 if (newProfileClicked) {
    try {
      ...
      // Use the ID from the JavaScript fallback to create a specific locator for the dialog
      let dialogContext = page;
      let dialogFound = false;
      let dialogLocator = page.locator('#pt1\\:oc_pg_pt\\:r1\\:2\\:ltbbk\\:oc_pnl_cmp\\:oc_scrn_pnl_tmpl\\:oc_scrn_tmpl_zm34g1\\:oc_scrn_pnl_pnl\\:oc_pnl_tmpl_zm34g1\\:oc_pnl_cntr');
      ...
      // Strategy 1: Check if the dialog is on the main page using the specific ID
      console.log('Trying to locate Guest Profile dialog on main page with specific ID...');
      dialogFound = await dialogLocator.isVisible({ timeout: 15000 }).catch(() => false);
      ...
      if (!dialogFound) {
        ...
        for (const iframeSelector of iframeSelectors) {
          ...
          const iframeLocator = page.frameLocator(iframeSelector);
          dialogLocator = iframeLocator.locator('#pt1\\:oc_pg_pt\\:r1\\:2\\:ltbbk\\:oc_pnl_cmp\\:oc_scrn_pnl_tmpl\\:oc_scrn_tmpl_zm34g1\\:oc_scrn_pnl_pnl\\:oc_pnl_tmpl_zm34g1\\:oc_pnl_cntr');
          dialogFound = await dialogLocator.isVisible({ timeout: 3000 }).catch(() => false);
          if (dialogFound) {
            dialogContext = iframeLocator;
      ...
      if (!dialogFound) {
        ...
        for (let i = 0; i < allIframes.length; i++) {
          ...
          if (dialogFound) {
            dialogContext = iframeLocator;
            console.log(`Found Guest Profile dialog in iframe: ${iframeName} (index ${i})`);
            break;
          }
        }
      }
      ...
      if (!dialogFound) {
        ...
        if (dialogInfo.length > 0) {
          dialogLocator = page.locator(`div[id="${dialogInfo[0].id}"]`).first();
          dialogFound = await dialogLocator.isVisible({ timeout: 5000 }).catch(() => false);
          if (dialogFound) {
    ...
    //---------------------------------------------------
     // Check if the dialog is open
 const { isOpen, dialogContext: guestProfileDialogContext, dialogLocator } = await checkGuestProfileDialog();const { isOpen, dialogContext: guestProfileDialogContext, dialogLocator } = await checkGuestProfileDialog();
if (!isOpen) {
  throw new Error('Guest Profile dialog did not open');
}

// Debug: Log all input fields in the dialog
const dialogInputs = await guestProfileDialogContext.evaluate(() => {
  const dialog = document.querySelector('#pt1\\:oc_pg_pt\\:r1\\:2\\:ltbbk\\:oc_pnl_cmp\\:oc_scrn_pnl_tmpl\\:oc_scrn_tmpl_zm34g1\\:oc_scrn_pnl_pnl\\:oc_pnl_tmpl_zm34g1\\:oc_pnl_cntr') ||
...
async function fillField(page, fieldLabel, value, guestProfileDialogContext) {
  console.log(`Filling in ${fieldLabel}...`);

  // Strategy 1: Use getByLabel with case-insensitive matching
  let field = guestProfileDialogContext.getByLabel(new RegExp(fieldLabel, 'i')).first();
  ...
  const alternateSelectors = [
    guestProfileDialogContext.locator(`input[id*="${fieldLabel.toLowerCase().replace(/\s/g, '')}"]`).first(),
    guestProfileDialogContext.locator(`input[aria-label*="${fieldLabel}" i]`).first(),
    guestProfileDialogContext.locator(`input[name*="${fieldLabel.toLowerCase().replace(/\s/g, '')}"]`).first(),
    guestProfileDialogContext.locator('input').filter({ has: guestProfileDialogContext.locator(`xpath=../*[contains(text(), "${fieldLabel}")]`) }).first(),
    // Additional variations for "First Name"
    ...(fieldLabel.toLowerCase().includes('first') ? [
        guestProfileDialogContext.locator('input[aria-label*="Given Name" i]').first(),
        guestProfileDialogContext.locator('input').filter({ has: guestProfileDialogContext.locator('text=/Given Name|First Name/i') }).first()
    ] : [])
  ];
  ...
  const fieldSet = await guestProfileDialogContext.evaluate((label, val) => {
...
// Fill the fields
try {
  await fillField(page, 'Last Name', clientName, guestProfileDialogContext);
  await fillField(page, 'First Name', firstName, guestProfileDialogContext);
} catch (error) {
 ...
 //--------------------------------------------------------------------   
        ...
        const mobileRow = guestProfileDialogContext.getByRole('row', { name: 'MOBILE Communication Type' });
        await page.waitForTimeout(2000);
        ...
        if (await mobileCommValue.isVisible({ timeout: 5000 })) {
          ...
          try {
            const gridCellInput = guestProfileDialogContext.getByRole('gridcell', { name: 'Communication Value Communication Value' })
          ...
          const anyCommValue = guestProfileDialogContext.locator('input[aria-label*="Communication Value"]').first();
        ...
        const saveButton = guestProfileDialogContext.getByRole('button', { name: 'Save and Select Profile' }).first();
        if (await saveButton.isVisible({ timeout: 5000 })) {
          ...
          await page.waitForTimeout(2000);
          const altSaveButton = guestProfileDialogContext.locator('button:has-text("Save and Select Profile")').first();
