    ...
    try {
      ...
      if (isProfileSearch) {
        ...
        if (hasAirBnBResult) {
          ...
          // Find and click Select button using the specific selector you provided
          console.log('Looking for Select button with exact CSS selector...');
          
          // Using the exact selector provided: 'span.x44s.x20 > div[role="presentation"][_afrgrp="0"]'
          const exactSelectSelector = page.locator('span.x44s.x20 > div[role="presentation"][_afrgrp="0"]');
          ...
          try {
            // Check if the exact selector is visible
            if (await exactSelectSelector.isVisible({ timeout: 5000 })) {
              console.log('Found Select button with exact CSS selector, clicking...');
              await exactSelectSelector.click();
              console.log('Clicked Select button with exact CSS selector');
            } else {
              // Fall back to other methods if specific selector isn't visible
              console.log('Exact CSS selector not visible, trying alternative selectors...');
