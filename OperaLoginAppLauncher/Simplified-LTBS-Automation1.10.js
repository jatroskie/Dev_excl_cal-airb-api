...
async function createBooking(clientName, firstName, telephoneNumber, roomNumber, 
  ...
  try {
    ...
    // Step 2: Select the rate row (avoid "Fixed Rate" checkbox)
    try {
      // Try the primary method first: Select the rate row by attribute
      ...
      console.log('Error selecting rate row:', rateRowError.message);
    }

    // Step 3: Click the Select button to confirm rate selection
    try {
      ...
      if (await rateSelectButton.isVisible({ timeout: 5000 })) {
        console.log('Found Select button for rate, clicking...');
        await page.waitForTimeout(1000);
        await rateSelectButton.click();
        console.log('Successfully clicked Select button for rate');
      } else {
        console.log('Rate Select button not found, trying alternate selector...');
        
        // Try alternate selector
        const altSelectButton = page.locator('button').filter({ hasText: 'Select' }).first();
        if (await altSelectButton.isVisible({ timeout: 3000 })) {
          console.log('Found alternate Select button, clicking...');
          await altSelectButton.click();
          console.log('Successfully clicked alternate Select button');
        } else {
    ...
    } catch (selectButtonError) {
      console.log('Error clicking Select button:', selectButtonError.message);
    }
