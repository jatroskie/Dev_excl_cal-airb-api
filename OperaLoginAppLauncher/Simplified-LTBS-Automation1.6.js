...
async function createBooking(clientName, firstName, telephoneNumber, roomNumber, 
  ...
  try {
    ...
    try {
      ...
      const doNotMoveCheckbox = page.getByRole('checkbox', { name: 'Do Not Move' }).first();
      if (await doNotMoveCheckbox.isVisible({ timeout: 3000 })) {
        ...
        if (await doNotMoveLabel.isVisible({ timeout: 3000 })) {
    ...
    if (newProfileClicked) {
      try {
        ...
        if (!dialogFound) {
          console.log('Guest Profile dialog not found with aria-label, trying iframe...');
          const iframeLocator = page.frameLocator('iframe[title="Content"]');
