...
async function createBooking(clientName, firstName, telephoneNumber, roomNumber, 
  ...
  try {
    ...
    // Extract the rate directly from the span element
    console.log('Extracting rate from screen...');
    let operaRate = 0;

    try {
      const rateElement = page.locator('span[id*="zm34g1"][class="x2d7"]').first();
      
      if (await rateElement.isVisible({ timeout: 3000 })) {
        ...
        console.log('Rate element not found with primary selector');
