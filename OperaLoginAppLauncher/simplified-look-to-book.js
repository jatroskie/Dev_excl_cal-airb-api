...
async function createBooking(clientName, firstName, telephoneNumber, roomNumber, 
  ...
  try {
    ...
    const reservationsItem = page.getByText('Reservations', { exact: true });
    ...
    const lookToBookItem = page.getByText('Look To Book Sales Screen');
    ...
    const searchIcon = page.locator('a[id*="oc_srclov_dummy_link"]');
    ...
    if (isProfileSearchPopup) {
      ...
      if (hasAirBnBResult) {
        ...
        await page.evaluate(() => {
          ...
          if (selectElements.length > 0) {
            ...
            try {
              ...
              if (selectElements[0].parentElement) {
                selectElements[0].parentElement.click();
                return true;
              }
            }
          }
        ...
        try {
          ...
          const iframeSelectButton = iframeLocator.getByRole('button', { name: 'Select' });
    ...
    // Fill arrival date
    const arrivalInput = page.getByRole('textbox', { name: 'Arrival' });
    ...
    // Fill departure date
    const departureInput = page.getByRole('textbox', { name: 'Departure' });
    ...
    // Set adults
    const adultsInput = page.getByRole('textbox', { name: 'Adults' });
    ...
    // Fill room number
    const roomInput = page.getByRole('textbox', { name: 'Room', exact: true });
    ...
    const rateSelectButton = page.getByRole('button', { name: 'Select', exact: true });
    ...
    try {
      const zarElement = page.locator('[id*="zm34g1"]').getByText('ZAR');
    ...
    // Exit booking
    const exitButton = page.getByRole('button', { name: 'Exit Booking' });
