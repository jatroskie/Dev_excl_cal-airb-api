...
async function createBooking(clientName, firstName, telephoneNumber, roomNumber, 
  ...
  try {
    ...
    // Updated "New Profile" click with precise locator
    console.log('Looking for New Profile link to create guest profile...');
    const newProfileLink = page.locator('span[id*="oc_pnl_tmpl_wqohv4:ss1:odec_srch_swtchr_advncd_sf:fe8:lnp"][class="x2d7"]').first();
    if (await newProfileLink.isVisible({ timeout: 10000 })) {
      ...
      const altNewProfileLink = page.getByRole('link', { name: 'New Profile' }).nth(0);
      if (await altNewProfileLink.isVisible({ timeout: 5000 })) {
        await altNewProfileLink.click();
        console.log('Clicked New Profile link using alternate selector');
        await page.waitForTimeout(3000);
      } else {
        console.log('New Profile link not found with any selector');
      }
    }
    
    // Handle the Guest Profile popup
    try {
      const profileDialog = page.getByRole('dialog', { name: 'Guest Profile' });
      await profileDialog.waitFor({ state: 'visible', timeout: 15000 }); // Increased timeout
      await page.waitForTimeout(2000);
    ...
    // Updated rate extraction logic
    console.log('Extracting rate from screen...');
    let operaRate = 0;

    try {
      // Target the "TOTAL COST OF STAY" section specifically
      const rateElement = page.locator('text="TOTAL COST OF STAY"').locator('xpath=following-sibling::*').first();
      ...
      if (await rateElement.isVisible({ timeout: 5000 })) {
        ...
        // Fallback: Look for any element containing "ZAR" and a number
        const rateElements = [
          page.locator('span[id*="zm34g1"][class="x2d7"]').first(),
        ...
        for (const element of rateElements) {
          if (await element.isVisible({ timeout: 2000 }).catch(() => false)) {
            const rateText = await element.textContent();
