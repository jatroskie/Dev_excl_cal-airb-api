 ...
 * and then uses keyboard navigation (Tab 14 times + Enter) to click the "Select" button in the popup.
 *
...
async function enterTravelAgentDetails(page, travelAgent, downloadsPath) {
    ...
    try {
        ...
        // --- Wait for the Popup and Use Keyboard Navigation to Click "Select" ---
        console.log('[enterTravelAgentDetails] Waiting for the popup to appear...');

        // Take a screenshot after clicking "Search" to debug the popup state
        await page.screenshot({ path: path.join(downloadsPath, 'after_search_button_click.png') });

        // Wait for the popup to appear by checking for a general popup container
   //     const popupLocator = page.locator('.x-window'); // Adjust this selector based on the popup’s container class
   //     await popupLocator.waitFor({ state: 'visible', timeout: 30000 });
  //      console.log('[enterTravelAgentDetails] Popup detected.');

        // Use keyboard navigation: Press Tab 14 times, then Enter
        await page.waitForTimeout(4000);
        console.log('[enterTravelAgentDetails] Using keyboard navigation to focus and click the "Select" button...');
        for (let i = 0; i < 14; i++) {
            await page.keyboard.press('Tab');
            await page.waitForTimeout(300); // Small delay between Tab presses to ensure UI updates
        }
        await page.keyboard.press('Enter');
        console.log('[enterTravelAgentDetails] "Select" button activated via keyboard navigation.');
